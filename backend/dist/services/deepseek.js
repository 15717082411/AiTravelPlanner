...
class DeepSeekService {
    constructor() {
        const { deepseekApiKey: apiKey } = (0, keys_1.getKeys)();
        if (!apiKey) {
    ...
    async extractPlanFields(text, currency = 'CNY') {
        try {
            ...
            const prefs = Array.isArray(data.preferences) ? data.preferences.map((x) => String(x)).filter(Boolean) : undefined;
    ...
    async extractExpenseFields(text, currency = 'CNY') {
        try {
            const response = await this.client.chat.completions.create({
                model: 'deepseek-chat',
                messages: [
    ...
    normalizeDate(d) {
        ...
        try {
            ...
            if (m2)
                return `${m2[1]}-${String(Number(m2[2])).padStart(2, '0')}-${String(Number(m2[3])).padStart(2, '0')}`;
            // 如果是合法日期字符串
            const dt = new Date(s);
            if (!isNaN(dt.getTime())) {
...
6. 每天安排3-5个主要活动`;
    ...
    validateAndFormatPlan(planData, request) {
        // 验证和格式化 AI 返回的数据
        return {
